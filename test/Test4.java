...
public class Test4 {
    ...
    public static void main(String[] args) {
        int m = 3;
        int n = 3;
        // int[][] cityMap = {{0, 2, 0, 0, 0, 2}, {0, 0, 2, 0, 1, 0}, {1, 0, 0, 2, 2,
        // 0}};
    ...
    static void dp(int[][] cityMap, int[][] route, int x, int y, int preX, int preY) {
        if (x == cityMap[0].length - 1 && y == cityMap.length - 1)
            return;
        ...
        for (int i = 0; i < direction.length; i++) {
            ...
            if (nextX >= cityMap[0].length || nextY >= cityMap.length)
                continue;
            if (cityMap[nextY][nextX] == 1)
                continue;
            else {
                if (cityMap[y][x] == 2) {
                    if (Math.abs(nextX - preX) - Math.abs(nextY - preY) == 0)
                        continue;
